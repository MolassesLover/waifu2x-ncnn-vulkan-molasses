...
static void print_usage() {
    ...
    fprintf(stdout, "  -m model-path        waifu2x model path (default=$HOME/.local/share/models-cunet)\n");
...
#endif
{
    ...
    path_t model = PATHSTR("$HOME/.local/share/models-cunet");
    ...
    if (model.find(PATHSTR("$HOME/.local/share/models-cunet")) != path_t::npos) {
        if (noise == -1) {
            prepadding = 18;
        } else if (scale == 1) {
            prepadding = 28;
    ...
    for (int i = 0; i < use_gpu_count; i++) {
        ...
        if (model.find(PATHSTR("$HOME/.local/share/models-cunet")) != path_t::npos) {
            if (heap_budget > 2600) tilesize[i] = 400;
            else if (heap_budget > 740) tilesize[i] = 200;
            else if (heap_budget > 250) tilesize[i] = 100;
            else tilesize[i] = 32;
        } else if (model.find(PATHSTR("models-upconv_7_anime_style_art_rgb")) != path_t::npos
                   || model.find(PATHSTR("models-upconv_7_photo")) != path_t::npos) {
            if (heap_budget > 1900) tilesize[i] = 400;
            else if (heap_budget > 550) tilesize[i] = 200;
            else if (heap_budget > 190) tilesize[i] = 100;
            else tilesize[i] = 32;
        }
    }
